...
**One-off events age off the board, but are never deleted.** `GET /api/one-off` only returns events whose `date` is within the last 7 days or in the future (`date >= today − 7d`, computed in local/TZ time); older rows stay in Postgres but stop appearing. Within that 7-day grace window, an event whose date+time has already passed renders **greyed-out** (struck-through, "PAST" pill, sorted below upcoming items) — see `renderOneOff` in `index.html`. So a past event lingers greyed for a week, then silently drops off the list. All CRUD is unaffected.

**Today's Agenda greys passed items too** (`loadToday` in `index.html`), mirroring the one-off table but with two differences: items are greyed **in place** (kept in chronological order, *not* sorted below upcoming ones), and "past" keys off each item's **end time** (or its start time when there's none, e.g. a one-off) — so an in-progress activity stays full-colour until it actually ends. Same visual treatment: muted card, struck-through title, "PAST" pill.
